...
const GrowthSummary = () => {
  ...
  return (
    <>
      ...
      <div className="flex flex-row w-[98rem] h-[14.8rem] pt-[3.2rem] pb-[2rem] bg-white rounded-tr-[2rem] rounded-tl-[2rem]">
        <div className="ml-[3.2rem]">{updatedLevelId && <ProfileStages size={96} />}</div> {/* updatedLevelId 사용 */}
        <div className="items-center ml-[1.6rem]">
          <span className="font-semibold text-[2.6rem] text-[#008A02]">{levelName}</span>
          <span className="font-semibold text-[2.6rem] ml-[1.6rem]">{nickname} 님의 정원</span>

          <div className="font-semibold text-[1.8rem] text-[#727272]">
