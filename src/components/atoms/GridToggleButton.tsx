...
const GridToggleButton: React.FC<GridToggleButtonProps> = ({ onClick, gridView }) => {
  return (
    ...
    >
      {gridView ? (
        <AiFillBook className="text-[4rem] text-green-400 sm:text-[2.5rem]" /> // gridView가 true일 때 AiFillBook 아이콘 렌더링
      ) : (
        <AiFillAppstore className="text-[4rem] text-green-400 sm:text-[2.5rem]" /> // gridView가 false일 때 AiFillAppstore 아이콘 렌더링
      )}
    </button>
  );
};
